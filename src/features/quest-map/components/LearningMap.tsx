...
export function LearningMap({ topic, region = 0, climate = 'temperature', selectedPlace = 0 }: { topic: MapTopic; region?: number; climate?: 'temperature' | 'snowfall'; selectedPlace?: number }) {
  ...
  const title = topic === 'regions' ? 'Virginia’s five geographic regions' : topic === 'climate' ? `Virginia ${climate === 'temperature' ? 'temperature' : 'snowfall'} patterns` : topic === 'population' ? 'Population density at five Virginia localities · 2020' : 'Virginia and its neighbors · relative location';
  ...
  return <figure className="learning-map">
    ...
    <svg viewBox="0 0 760 400" role="img" aria-label={title}>
      ...
      {topic === 'population' ? POPULATION_PLACES.map((place, i) => { const p = point(place.at[0], place.at[1]); return <g key={place.name}><circle cx={p.x} cy={p.y} r={i === selectedPlace ? 16 : 13} fill={DENSITY_COLORS[place.category]} stroke={i === selectedPlace ? '#102d2b' : '#fff'} strokeWidth="3" /><text x={p.x} y={p.y + 5} textAnchor="middle" fill={place.category >= 4 ? '#fff' : '#1c332f'} fontSize="14" fontWeight="700">{i + 1}</text></g>; }) : null}
    ...
    <p className="school-note">{topic === 'regions' ? 'Simplified region bands; boundaries are approximate. Numbers match the region buttons below.' : topic === 'climate' ? 'Concept map adapted from the classroom patterns. Colors do not represent measured temperature or snowfall ranges; local conditions vary.' : topic === 'population' ? 'Circles locate selected places; their colors describe the whole locality’s average. Unmarked land has no data on this map, not zero people.' : 'Neighbor labels show direction, not state shapes or exact border positions. ★ marks the state capital.'} Land outline: U.S. Census Bureau, 2024.</p>
  </figure>;
}
