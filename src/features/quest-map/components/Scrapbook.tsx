...
import type { ExplorePlace } from '../../../contexts/published-content/adapters/explorePlaces';

const ScrapbookExplorer = lazy(() => import('./ScrapbookExplorer').then(module => ({ default: module.ScrapbookExplorer })));
...
export function Scrapbook({ onClose }: { onClose: () => void }) {
  ...
  function choosePlace(place: ExplorePlace) {
    const target = pageForPlace(currentBook.current, place.name, active);
    if (!target) { setStatus('All five pages are in progress. Open My pages to choose a page to edit.'); return; }
    if (!target.existing) updateVisit({ place: place.name, region: REGION_LESSONS[place.region].name, jurisdiction: 'VA' }, target.index);
    setActive(target.index); setExploring(false); setPreview(false);
    setStatus(target.existing ? `Opened your existing page for ${place.name}.` : `${place.name} is planned on visit ${target.index + 1}. Add your own story after your visit.`);
  ...
  return <PageContent label="My Virginia scrapbook" titleId="scrapbook-title" className="school-workspace scrapbook-workspace" onClose={onClose}>
    <div className={`scrapbook-controls ${exploring ? "scrapbook-exploring-header" : ""}`}>
      ...
      <div className="school-actions">
        <button type="button" className="secondary-action" onClick={() => { setExploring(false); setPreview(!preview); }}><BookOpen aria-hidden="true" />{preview ? 'Keep writing' : 'Preview & present'}</button>
    ...
    <nav className="scrapbook-mode-nav scrapbook-controls" aria-label="Scrapbook workspace">
      <button type="button" aria-pressed={exploring} onClick={() => { setExploring(true); setPreview(false); }}><Compass aria-hidden="true" /><span>Explore places<small>70 stories · 5 regions · a map full of ideas</small></span></button>
      <button type="button" aria-pressed={!exploring && !preview} onClick={() => { setExploring(false); setPreview(false); }}><BookOpen aria-hidden="true" /><span>My pages<small>Plan, write, and keep your memories</small></span></button>
    </nav>
    {exploring ? <Suspense fallback={<p role="status">Opening your discovery map…</p>}><ScrapbookExplorer onChoose={choosePlace} actionFor={place => {
      const target = pageForPlace(book, place.name, active);
      return { disabled: !target || photoBusy, label: !target ? 'All five pages are in progress' : target.existing ? `Open my visit ${target.index + 1} page` : `Plan on visit ${target.index + 1}` };
    }} /></Suspense> : preview ? <div className="scrapbook-preview">
      ...
      <div className="scrapbook-editor">
        <h3 ref={editorHeading} tabIndex={-1}>Visit {active + 1}: {v.place || 'Start a new discovery'}</h3>
        ...
        <details className="school-details"><summary>Need a place to explore?</summary><p>Find historical sites, museums, and natural landmarks across Virginia's five regions.</p><button type="button" className="secondary-action" onClick={() => { setExploring(true); setPreview(false); }}>Open the discovery map</button><p>Natural places need a story explaining their historical importance.</p><a href="https://www.nps.gov/kids/every-kid-outdoors.htm" target="_blank" rel="noreferrer">Explore the fourth-grade Every Kid Outdoors pass ↗</a><p>The pass covers eligible entrance or day-use fees at participating federal sites. Check official details with an adult.</p></details>
    ...
    {!exploring && <section className="school-details scrapbook-controls"><h3>Ready-to-share checklist</h3><p>A planning guide based on your assignment and rubric. Completed fields are not a grade; reread for accuracy and detail.</p>
